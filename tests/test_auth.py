...
def test_auth_invalid_token(client):
    response = client.get(
        "/favorites", headers={"Authorization": "Bearer invalid_token"}
    )
...
def test_valid_token_returns_200(client):
    response = client.get("/favorites", headers={"Authorization": "Bearer 1234567890"})
...
def test_unauthorized_role_returns_403(client):
    response = client.get(
        "/favorites", headers={"Authorization": "Bearer unauthorized_token"}
    )
    assert response.status_code == 403
    assert response.json == {"error": "Invalid token"}
...
def test_valid_token_without_role_returns_403(client):
    ...
    response = client.get(
        "/favorites", headers={"Authorization": f"Bearer {USER_TOKEN}"}
    )
    assert response.status_code == 403
